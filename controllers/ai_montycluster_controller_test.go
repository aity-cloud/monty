...
var _ = Describe("AI MontyCluster Controller", Ordered, Label("controller"), func() {
	cluster := &aiv1beta1.MontyCluster{}

	createCluster := func(c *aiv1beta1.MontyCluster) {
		...
		c.Spec.Opensearch = &montymeta.OpensearchClusterRef{
			Name:      "test-cluster",
			Namespace: c.Namespace,
		}
		...
		nats := &montycorev1beta1.NatsCluster{
			...
			Spec: montycorev1beta1.NatsSpec{
				...
				AuthMethod: montycorev1beta1.NatsAuthNkey,
			},
		}
	...
	It("should create the necessary monty service deployments", func() {
		...
		wg := &sync.WaitGroup{}
		createCluster(buildAICluster(montyClusterOpts{Name: "test"}))
	...
	It("should not create the metrics service when the prometheus endpoint is invalid", func() {
		By("waiting for the cluster to be created")
		createCluster(buildAICluster(montyClusterOpts{
			Name:               "test",
	...
	Specify("providing hyperparameters should work", func() {
		...
		updateObject(cluster, func(c *aiv1beta1.MontyCluster) {
	...
	It("should allow adding pretrained models to an existing montycluster", func() {
		By("creating an montycluster without any models")
		createCluster(buildAICluster(montyClusterOpts{
			Name: "test-cluster",
		}))
		...
		By("adding the model to the montycluster")
		updateObject(cluster, func(c *aiv1beta1.MontyCluster) {
	...
	It("should handle editing models in an existing montycluster", func() {
		By("creating a model")
		ns := makeTestNamespace()
		model := aiv1beta1.PretrainedModel{
			ObjectMeta: metav1.ObjectMeta{
		...
		By("creating an montycluster with the model")
		c := buildAICluster(montyClusterOpts{
			Name:      "test-cluster",
			Namespace: ns,
		})
		c.Spec.Services.Inference.PretrainedModels =
		...
		By("deleting the model from the montycluster")
		updateObject(cluster, func(c *aiv1beta1.MontyCluster) {
			c.Spec.Services.Inference.PretrainedModels =
		...
		By("adding the model back to the montycluster")
		updateObject(cluster, func(c *aiv1beta1.MontyCluster) {
			c.Spec.Services.Inference.PretrainedModels =
				append(c.Spec.Services.Inference.PretrainedModels,
					corev1.LocalObjectReference{
	...
	It("should handle invalid pretrained models", func() {
		By("creating an montycluster with an invalid pretrained model")
		createCluster(buildAICluster(montyClusterOpts{
			Name:   "test-cluster",
		...
		Eventually(Object(cluster)).Should(MatchStatus(
			func(s aiv1beta1.MontyClusterStatus) bool {
				return s.State == aiv1beta1.MontyClusterStateError &&
					len(s.Conditions) > 0
			},
		))
		<-done
	...
	It("should reconcile internal s3 resources", func() {
		...
		Eventually(Object(cluster)).Should(MatchStatus(func(status aiv1beta1.MontyClusterStatus) bool {
	...
	It("should reconcile external s3 resources", func() {
		By("creating the montycluster with a missing secret")
		c := buildAICluster(montyClusterOpts{
			Name: "test",
		})
		...
		Eventually(Object(cluster), 60*time.Second).Should(MatchStatus(func(status aiv1beta1.MontyClusterStatus) bool {
	...
	It("should reconcile internal s3 resources", func() {
		...
		By("checking that s3 info in the cluster status should be set")
		Eventually(Object(cluster)).Should(MatchStatus(func(status aiv1beta1.MontyClusterStatus) bool {
			return status.Auth.S3Endpoint == fmt.Sprintf("http://monty-seaweed-s3.%s.svc", cluster.Namespace) &&
				status.Auth.S3AccessKey != nil &&
				status.Auth.S3SecretKey != nil
		}))
	})
