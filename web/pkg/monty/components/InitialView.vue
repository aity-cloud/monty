...
export default {
  ...
  data() {
    const installSteps = [
      {
        name:  'repo',
        label: 'Install Monty Repository',
        ready: false,
      },
      {
        name:  'install',
        label: 'Install Monty Chart',
        ready: false,
      },
    ];
...
<template>
  ...
  <div v-else class="container">
    <div v-if="!repo && !controllerChart && !install" class="title p-10 center">
      <h1 class="mb-20">
        <img
          src="../assets/images/monty-icon.svg"
          height="64"
        />
        {{ t("monty.title") }}
      </h1>
    ...
    >
      ...
      <template #repo>
        <div class="mt-20 mb-20 center">
          <h2>
            Monty Repository Install
          </h2>
          <p>
            Install the Monty Repository by clicking the button below.
          </p>
          <AsyncButton
            class="mt-40"
            mode="edit"
        ...
        <template v-else>
          <div class="mt-20 mb-20 center">
            ...
            <div v-if="isChartPartlyInstalled">
              <p>
                It looks like the Monty chart installation has already begun but hasn't finished yet.<br />You can take a look at the status of the installation <a href="#" @click.prevent="goToApps">here</a>.
              </p>
            </div>
            <div v-else>
              <p>
                Install the Monty chart by clicking the button below.
              </p>
              <button
                class="btn role-primary mt-40"
                @click.prevent="chartRoute"
              >
                {{ t("monty.installChart") }}
              </button>
            </div>
          </div>
        </template>
