...
export async function createAlertEndpoint(endpoint: AlertEndpoint) {
  await axios.post('monty-api/AlertEndpoints/configure', endpoint);
}

export async function updateAlertEndpoint(endpoint: UpdateAlertEndpointRequest) {
  await axios.put('monty-api/AlertEndpoints/configure', endpoint);
}

export async function getAlertEndpoints(vue: any): Promise<Endpoint[]> {
  const response = (await axios.get <AlertEndpointList>('monty-api/AlertEndpoints/list')).data;
...
export async function getAlertEndpoint(id: string, vue: any): Promise<Endpoint> {
  const response = (await axios.post<AlertEndpoint>(`monty-api/AlertEndpoints/list/${ id }`, { id })).data;

  return new Endpoint({ id: { id }, endpoint: response }, vue);
}

export async function testAlertEndpoint(request: Reference) {
  await axios.post<AlertEndpoint>(`monty-api/AlertNotifications/test`, request);
}

export function deleteEndpoint(id: string) {
  return axios.post(`monty-api/AlertEndpoints/delete`, { id: { id }, forceDelete: false });
}

export function createAlertCondition(alertCondition: AlertCondition) {
  return axios.post(`monty-api/AlertConditions/configure`, alertCondition);
}
...
export async function getAlertConditionsWithStatus(vue: any, itemFilter?: { clusters?: string[], groupIds?: string[], alertTypes?: string[] }, clusters?: Cluster[]) {
  const response = (await axios.post<ListStatusResponse>('monty-api/AlertConditions/list/withStatus', { itemFilter })).data;
...
export async function cloneAlertCondition(condition: AlertCondition, clusterIds: string[]) {
  ...
  (await axios.post<AlertConditionList>('monty-api/AlertConditions/clone', body));
}

export async function updateAlertCondition(condition: UpdateAlertConditionRequest): Promise<any> {
  return await axios.put('monty-api/AlertConditions/configure', condition);
}

export async function getAlertConditionChoices(request: AlertDetailChoicesRequest): Promise<ListAlertTypeDetails> {
  return (await axios.post<ListAlertTypeDetails>('monty-api/AlertConditions/choices', request)).data;
}

export function deleteAlertCondition(id: string, groupId : string) {
  return axios.delete(`monty-api/AlertConditions/configure`, { data: { id, groupId } });
}

export async function getAlertConditionStatus(id: string): Promise<AlertStatusResponse> {
  return (await axios.post(`monty-api/AlertConditions/status/${ id }`, { id })).data;
}

export function silenceAlertCondition(request: SilenceRequest) {
  return axios.post(`monty-api/AlertConditions/silences`, request);
}

export function deactivateSilenceAlertCondition(id: ConditionReference) {
  return axios.delete(`monty-api/AlertConditions/silences`, { data: id });
}

export async function getConditionTimeline(request: TimelineRequest): Promise<TimelineResponse> {
  return (await axios.post<TimelineResponse>(`monty-api/AlertConditions/timeline`, request)).data;
}

export async function getAlarmNotifications(request: ListAlarmMessageRequest): Promise<ListMessageResponse> {
  return (await axios.post<ListMessageResponse>(`monty-api/AlertNotifications/alarms/list`, request)).data;
}

export async function getAlertConditionGroups(): Promise<Reference[]> {
  return (await axios.get(`monty-api/AlertConditions/groups`)).data?.items || [];
}
...
export async function getClusterConfiguration(): Promise<ClusterConfiguration> {
  return (await axios.get<ClusterConfiguration>(`monty-api/AlertingAdmin/configuration`)).data;
}
...
export async function configureCluster(config: ClusterConfiguration) {
  try {
    await axios.post(`monty-api/AlertingAdmin/configure`, config);
  } catch (ex) {
...
export async function getClusterStatus(): Promise<InstallStatus> {
  return (await axios.get<InstallStatus>(`monty-api/AlertingAdmin/status`)).data;
}

export function installCluster() {
  return axios.post(`monty-api/AlertingAdmin/install`);
}
export function uninstallCluster() {
  return axios.post(`monty-api/AlertingAdmin/uninstall`);
}
