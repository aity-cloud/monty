...
# Third Party
from monty_proto.log_anomaly_payload_pb import PayloadList
import pandas as pd
...
pretrained_model_script_source += "ctx._source.montylog_confidence = params['montylog_score'];"
workload_script_source += "ctx._source.montylog_confidence = params['montylog_score'];"
script_for_anomaly = (
    "ctx._source.anomaly_predicted_count += 1; ctx._source.montylog_anomaly = true;"
)
...
async def update_logs(es, df):
    ...
    model_keywords_dict = {"drain":  ["_id", "masked_log", "template_matched","template_cluster_id","inference_model", "anomaly_level"],
                          "montylog":  ["_id", "masked_log", "anomaly_level", "template_matched","template_cluster_id","montylog_confidence", "inference_model"]}
...
async def init_nats():
    from monty_nats import NatsWrapper
