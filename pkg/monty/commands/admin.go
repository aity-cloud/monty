	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(clusters) == 0 {
				...
				// Important to validate here !! Since cortex has no knowledge of available monty clusters,
...
func BuildListRulesCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			// since cortexadmin Server no longer embeds an monty management client,
