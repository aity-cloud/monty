...
func (in *GatewayConfigSpec) UnredactSecrets(unredacted *GatewayConfigSpec) error {
	...
	if err := in.Dashboard.UnredactSecrets(unredacted.GetDashboard()); errors.IsDiscontinuity(err) {
	...
	if err := in.Storage.UnredactSecrets(unredacted.GetStorage()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
	...
	if err := in.Certs.UnredactSecrets(unredacted.GetCerts()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
	...
	if err := in.Auth.UnredactSecrets(unredacted.GetAuth()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
...
func (in *DashboardServerSpec) UnredactSecrets(unredacted *DashboardServerSpec) error {
	...
	var details []protoiface.MessageV1
	if err := in.WebCerts.UnredactSecrets(unredacted.GetWebCerts()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
...
func (in *StorageSpec) UnredactSecrets(unredacted *StorageSpec) error {
	...
	var details []protoiface.MessageV1
	if err := in.Etcd.UnredactSecrets(unredacted.GetEtcd()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
...
func (in *PluginsSpec) FlagSet(prefix ...string) *pflag.FlagSet {
	...
	fs.SortFlags = true
	fs.Var(flagutil.StringPtrValue(flagutil.Ptr("/var/lib/monty/plugins"), &in.Dir), strings.Join(append(prefix, "dir"), "."), "Directory to search for plugin binaries.")
	if in.Filters == nil {
...
func (in *FilesystemCacheSpec) FlagSet(prefix ...string) *pflag.FlagSet {
	...
	fs.SortFlags = true
	fs.Var(flagutil.StringPtrValue(flagutil.Ptr("/var/lib/monty/plugin-cache"), &in.Dir), strings.Join(append(prefix, "dir"), "."), "Directory to store plugin binaries and patches in.")
	return fs
}
...
func (in *AuthSpec) UnredactSecrets(unredacted *AuthSpec) error {
	...
	var details []protoiface.MessageV1
	if err := in.Openid.UnredactSecrets(unredacted.GetOpenid()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
