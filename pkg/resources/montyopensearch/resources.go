...
const (
	MontyPreprocessingInstanceName = "monty"
)
...
func (r *Reconciler) buildOTELPreprocessor() runtime.Object {
	otel := &loggingv1beta1.Preprocessor{
		ObjectMeta: metav1.ObjectMeta{
			Name:      MontyPreprocessingInstanceName,
			Namespace: r.instance.Namespace,
		},
...
func calculateImage(repo, version, opensearchVersion string) string {
	...
	montyVersion, err := semver.NewVersion(version)
	if err != nil {
		return fmt.Sprintf(
			"%s/opensearch:v%s-%s",
			repo,
	...
	if newVersionConstraint.Check(montyVersion) {
		return fmt.Sprintf(
			"%s/opensearch:v%s-%s",
			repo,
			version,
