...
import (
	...
	esapiext "github.com/aity-cloud/monty/pkg/opensearch/opensearch/types"
		...
		err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			...
			if op.ShouldRequeue() {
				if retErr != nil {
					...
					r.cluster.Status.IndexState = aiv1beta1.MontyClusterStateError
				} else {
					// If no error occurred, but we need to requeue, the state should be
					// set to working
					r.cluster.Status.IndexState = aiv1beta1.MontyClusterStateWorking
				}
			} else if len(r.cluster.Status.Conditions) == 0 {
				// If we are not requeueing and there are no conditions, the state should
				// be set to ready
				r.cluster.Status.IndexState = aiv1beta1.MontyClusterStateReady
			}
