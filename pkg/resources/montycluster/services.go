...
func (r *Reconciler) montyServices() ([]resources.Resource, error) {
...
func (r *Reconciler) pretrainedModels() (resourceList []resources.Resource, retError error) {
	...
	desiredModels := r.montyCluster.Spec.Services.Inference.PretrainedModels
...
func (r *Reconciler) pretrainedModelDeployment(
	...
	return func() (runtime.Object, reconciler.DesiredState, error) {
		...
		ctrl.SetControllerReference(r.montyCluster, deployment, r.client.Scheme())
...
func (r *Reconciler) workloadDrainDeployment() (resources.Resource, error) {
	return func() (runtime.Object, reconciler.DesiredState, error) {
		...
		if lo.FromPtrOr(r.montyCluster.Spec.Services.Drain.Workload.Enabled, false) && lo.FromPtrOr(r.montyCluster.Spec.Services.Drain.Enabled, true) {
...
func (r *Reconciler) gpuWorkerContainer() corev1.Container {
	...
	if r.montyCluster.Spec.S3.NulogS3Bucket != "" {
		envVars = append(envVars,
			corev1.EnvVar{
				Name:  "S3_BUCKET",
				Value: r.montyCluster.Spec.S3.NulogS3Bucket,
			})
	}
	return corev1.Container{
...
func (r *Reconciler) genericEnvAndVolumes() (
	...
	newEnvVars, newVolumeMounts, newVolumes := nats.ExternalNatsObjects(
		...
		types.NamespacedName{
			Name:      r.montyCluster.Spec.NatsRef.Name,
			Namespace: r.montyCluster.Namespace,
		},
	)
	...
	envVars = append(envVars, corev1.EnvVar{
		...
		ValueFrom: &corev1.EnvVarSource{
			SecretKeyRef: r.montyCluster.Status.Auth.OpensearchAuthSecretKeyRef,
		},
	})
	return
}
...
func (r *Reconciler) s3EnvVars() (envVars []corev1.EnvVar) {
	...
	if r.montyCluster.Status.Auth.S3AccessKey != nil &&
		r.montyCluster.Status.Auth.S3SecretKey != nil &&
		r.montyCluster.Status.Auth.S3Endpoint != "" {
		envVars = append(envVars, corev1.EnvVar{
			Name: "S3_ACCESS_KEY",
			ValueFrom: &corev1.EnvVarSource{
				SecretKeyRef: r.montyCluster.Status.Auth.S3AccessKey,
			},
		}, corev1.EnvVar{
			Name: "S3_SECRET_KEY",
			ValueFrom: &corev1.EnvVarSource{
				SecretKeyRef: r.montyCluster.Status.Auth.S3SecretKey,
			},
		}, corev1.EnvVar{
			Name:  "S3_ENDPOINT",
			Value: r.montyCluster.Status.Auth.S3Endpoint,
		})
	}
	return envVars
}
...
func (r *Reconciler) nulogHyperparameters() (runtime.Object, reconciler.DesiredState, error) {
	...
	if len(r.montyCluster.Spec.NulogHyperparameters) > 0 {
		data = r.montyCluster.Spec.NulogHyperparameters
	} else {
	...
	cm, err := hyperparameters.GenerateHyperparametersConfigMap("nulog", r.montyCluster.Namespace, data)
	if err != nil {
		return nil, nil, err
	}
	cm.Labels["monty-service"] = "nulog"
	ctrl.SetControllerReference(r.montyCluster, &cm, r.client.Scheme())
...
func (r *Reconciler) inferenceDeployment() (runtime.Object, reconciler.DesiredState, error) {
	...
	return deployment, deploymentStateDisabled(r.montyCluster.Spec.Services.Inference.Enabled), nil
}
...
func (r *Reconciler) drainDeployment() (runtime.Object, reconciler.DesiredState, error) {
	...
	s3EnvVars := r.s3EnvVars()
	deployment.Spec.Template.Spec.Containers[0].Env = append(deployment.Spec.Template.Spec.Containers[0].Env, s3EnvVars...)
	if r.montyCluster.Spec.S3.DrainS3Bucket != "" {
		deployment.Spec.Template.Spec.Containers[0].Env = append(deployment.Spec.Template.Spec.Containers[0].Env,
			corev1.EnvVar{
				Name:  "S3_BUCKET",
				Value: r.montyCluster.Spec.S3.DrainS3Bucket,
			})
	}
	deployment.Spec.Replicas = r.montyCluster.Spec.Services.Drain.Replicas
	return deployment, deploymentState(r.montyCluster.Spec.Services.Drain.Enabled), nil
}
...
func (r *Reconciler) trainingControllerDeployment() (runtime.Object, reconciler.DesiredState, error) {
	...
	s3EnvVars := r.s3EnvVars()
	deployment.Spec.Template.Spec.Containers[0].Env = append(deployment.Spec.Template.Spec.Containers[0].Env, s3EnvVars...)
	return deployment, deploymentStateDisabled(r.montyCluster.Spec.Services.TrainingController.Enabled), nil
}
...
func (r *Reconciler) payloadReceiverDeployment() (runtime.Object, reconciler.DesiredState, error) {
	...
	return deployment, deploymentState(r.montyCluster.Spec.Services.PayloadReceiver.Enabled), nil
}
...
func (r *Reconciler) payloadReceiverService() (runtime.Object, reconciler.DesiredState, error) {
	...
	return service, deploymentState(r.montyCluster.Spec.Services.PayloadReceiver.Enabled), nil
}
...
func (r *Reconciler) preprocessingDeployment() (runtime.Object, reconciler.DesiredState, error) {
	...
	deployment.Spec.Replicas = r.montyCluster.Spec.Services.Preprocessing.Replicas
	return deployment, deploymentState(r.montyCluster.Spec.Services.Preprocessing.Enabled), nil
}
...
func (r *Reconciler) gpuCtrlDeployment() (runtime.Object, reconciler.DesiredState, error) {
	...
	deployment.Spec.Template.Spec.RuntimeClassName = r.montyCluster.Spec.Services.GPUController.RuntimeClass
	...
	insertHyperparametersVolume(deployment, "nulog")
	return deployment, deploymentStateDisabled(r.montyCluster.Spec.Services.GPUController.Enabled), nil
}

func (r *Reconciler) getPrometheusEndpoint() (endpoint string) {
	lg := log.FromContext(r.ctx)
	if r.montyCluster.Spec.Services.Metrics.Enabled != nil && !*r.montyCluster.Spec.Services.Metrics.Enabled {
		return
	}
	if r.montyCluster.Spec.Services.Metrics.PrometheusReference != nil {
		prometheus := &monitoringv1.Prometheus{}
		err := r.client.Get(r.ctx, types.NamespacedName{
			Name:      r.montyCluster.Spec.Services.Metrics.PrometheusReference.Name,
			Namespace: r.montyCluster.Spec.Services.Metrics.PrometheusReference.Namespace,
		}, prometheus)
		if err != nil {
	...
	if r.montyCluster.Spec.Services.Metrics.PrometheusEndpoint != "" {
		endpoint = r.montyCluster.Spec.Services.Metrics.PrometheusEndpoint
	}
...
func (r *Reconciler) metricsDeployment() (runtime.Object, reconciler.DesiredState, error) {
	...
	if err != nil && (r.montyCluster.Spec.Services.Metrics.Enabled == nil || *r.montyCluster.Spec.Services.Metrics.Enabled) {
		return deployment, deploymentState(r.montyCluster.Spec.Services.Metrics.Enabled), fmt.Errorf("prometheus endpoint is not a valid URL: %s", prometheusEndpoint)
	}
	deployment.Spec.Template.Spec.Containers[0].Env = append(deployment.Spec.Template.Spec.Containers[0].Env, corev1.EnvVar{
		Name:  "PROMETHEUS_ENDPOINT",
		Value: prometheusEndpoint,
	})
	for _, extraVolume := range r.montyCluster.Spec.Services.Metrics.ExtraVolumeMounts {
	...
	return deployment, deploymentState(r.montyCluster.Spec.Services.Metrics.Enabled), nil
}
...
func (r *Reconciler) metricsService() (runtime.Object, reconciler.DesiredState, error) {
	...
	ctrl.SetControllerReference(r.montyCluster, service, r.client.Scheme())
	return service, deploymentState(r.montyCluster.Spec.Services.Metrics.Enabled), nil
}
...
func (r *Reconciler) metricsServiceMonitor() (runtime.Object, reconciler.DesiredState, error) {
	...
	ctrl.SetControllerReference(r.montyCluster, serviceMonitor, r.client.Scheme())
	if r.montyCluster.Spec.Services.Metrics.PrometheusReference == nil {
		return serviceMonitor, reconciler.StateAbsent, nil
	}
	return serviceMonitor, deploymentState(r.montyCluster.Spec.Services.Metrics.Enabled), nil
}
...
func (r *Reconciler) metricsPrometheusRule() (runtime.Object, reconciler.DesiredState, error) {
					...
					Rules: []monitoringv1.Rule{
						{
							...
							Annotations: map[string]string{
								...
								"description": "Monty has calculated an alert score >= 5 for cluster CPU usage",
							},
						},
						{
							...
							Annotations: map[string]string{
								...
								"description": "Monty has calculated an alert score >= 5 for cluster memory usage",
							},
						},
					},
				},
	...
	if (r.montyCluster.Spec.Services.Metrics.Enabled == nil || *r.montyCluster.Spec.Services.Metrics.Enabled) &&
		r.montyCluster.Spec.Services.Metrics.PrometheusReference != nil {
		prometheus := &monitoringv1.Prometheus{}
		err := r.client.Get(r.ctx, types.NamespacedName{
			Name:      r.montyCluster.Spec.Services.Metrics.PrometheusReference.Name,
			Namespace: r.montyCluster.Spec.Services.Metrics.PrometheusReference.Namespace,
		...
		if err != nil {
			return prometheusRule, deploymentState(r.montyCluster.Spec.Services.Metrics.Enabled), err
		}
		if prometheus.Spec.RuleNamespaceSelector == nil {
			...
			err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
				...
				if r.montyCluster.Status.PrometheusRuleNamespace != prometheus.Namespace {
					r.montyCluster.Status.PrometheusRuleNamespace = prometheus.Namespace
					err = r.client.Status().Update(r.ctx, r.montyCluster)
					if err != nil {
						return err
					}
				}
				if r.montyCluster.DeletionTimestamp == nil {
					controllerutil.AddFinalizer(r.montyCluster, prometheusRuleFinalizer)
			...
			ctrl.SetControllerReference(r.montyCluster, prometheusRule, r.client.Scheme())
		}
		...
		return prometheusRule, deploymentState(r.montyCluster.Spec.Services.Metrics.Enabled), nil
	}
...
func (r *Reconciler) opensearchUpdateDeployment() (runtime.Object, reconciler.DesiredState, error) {
	...
	return deployment, deploymentState(r.montyCluster.Spec.Services.OpensearchUpdate.Enabled), nil
}

func (r *Reconciler) generateSHAID() string {
	hash := sha1.New()
	hash.Write([]byte(r.montyCluster.Name + r.montyCluster.Namespace))
	sum := hash.Sum(nil)
...
func (r *Reconciler) externalOpensearchConfig() (retResources []resources.Resource, retErr error) {
	retErr = retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		r.montyCluster.Status.Auth.OpensearchAuthSecretKeyRef = &corev1.SecretKeySelector{
			Key: "password",
			LocalObjectReference: corev1.LocalObjectReference{
