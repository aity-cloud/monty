...
func BuildManagementCmd() *cobra.Command {
	...
	cliutil.AddSubcommands(cmd, append([]*cobra.Command{
		...
		BuildManagementWatchClustersCmd(),
		...
		BuildManagementWatchClusterHealthStatusCmd(),
...
func BuildManagementWatchClustersCmd() *cobra.Command {
...
`[1:],
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			response, err := client.WatchClusters(cmd.Context(), in)
			if err != nil {
				return err
			}
...
func BuildManagementWatchClusterHealthStatusCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "watch-cluster-health-status",
		Short: "",
		Long: `
HTTP handlers for this method:
- GET /watch/health
`[1:],
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			response, err := client.WatchClusterHealthStatus(cmd.Context(), &emptypb.Empty{})
			if err != nil {
				return err
			}
			return cli.RenderStreamingOutput(cmd, response)
...
func (in *WatchClustersRequest) FlagSet(prefix ...string) *pflag.FlagSet {
	fs := pflag.NewFlagSet("WatchClustersRequest", pflag.ExitOnError)
	fs.SortFlags = true
	if in.KnownClusters == nil {
		in.KnownClusters = &v1.ReferenceList{}
	}
	fs.AddFlagSet(in.KnownClusters.FlagSet(append(prefix, "known-clusters")...))
	return fs
}
