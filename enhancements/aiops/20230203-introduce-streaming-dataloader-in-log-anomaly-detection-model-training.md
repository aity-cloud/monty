...
* Data-preprocessing function: 
    ...
    - step1: Masking. Apply [Opni's pre-defined masking rules](https:// github.com/aity-cloud/monty-inference-service/blob/main/opni_inference_service/models/opnilog/masker.py) to logs.
...
* Tokenize and padding batches. 
    - Tokenization. nothing new but just the [tokenization method](https:// github.com/aity-cloud/monty-inference-service/blob/main/opni_inference_service/models/opnilog/opnilog_tokenizer.py). For each log as input, tokenize the log to a lost of tokens, map each token to an integer(its `index`), return the list of token-indices.
