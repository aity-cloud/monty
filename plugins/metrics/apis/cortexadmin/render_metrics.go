...
type MetricsNodeConfigInfo struct {
	Id            string
	HasCapability bool
	Spec          *node.MetricsCapabilityConfig
	IsDefault     bool
}
...
func RenderMetricsNodeConfigs(nodes []MetricsNodeConfigInfo, defaultConfig *node.MetricsCapabilityConfig) string {
	writer := table.NewWriter()
	writer.SetStyle(table.StyleColoredDark)
...
func RenderDefaultNodeConfig(defaultConfig *node.MetricsCapabilityConfig) string {
	marshal := prototext.MarshalOptions{
		Multiline: true,
		Indent:    "  ",
		EmitASCII: true,
