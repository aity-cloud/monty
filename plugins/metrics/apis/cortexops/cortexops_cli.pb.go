...
func (in *CapabilityBackendConfigSpec) UnredactSecrets(unredacted *CapabilityBackendConfigSpec) error {
	...
	var details []protoiface.MessageV1
	if err := in.CortexConfig.UnredactSecrets(unredacted.GetCortexConfig()); errors1.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
...
func (in *CortexApplicationConfig) UnredactSecrets(unredacted *CortexApplicationConfig) error {
	...
	var details []protoiface.MessageV1
	if err := in.Storage.UnredactSecrets(unredacted.GetStorage()); errors1.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
...
func (in *ResetRequest) UnredactSecrets(unredacted *ResetRequest) error {
	...
	var details []protoiface.MessageV1
	if err := in.Patch.UnredactSecrets(unredacted.GetPatch()); errors1.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
