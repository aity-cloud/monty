// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.8
// 	protoc        (unknown)
// source: github.com/aity-cloud/monty/plugins/example/pkg/example/example.proto

package example

import (
	validate "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	v1 "github.com/aity-cloud/monty/pkg/apis/core/v1"
	driverutil "github.com/aity-cloud/monty/pkg/plugins/driverutil"
	_ "github.com/kralicky/codegen/cli"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	fieldmaskpb "google.golang.org/protobuf/types/known/fieldmaskpb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type EchoRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EchoRequest) Reset() {
	*x = EchoRequest{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EchoRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EchoRequest) ProtoMessage() {}

func (x *EchoRequest) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EchoRequest.ProtoReflect.Descriptor instead.
func (*EchoRequest) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{0}
}

func (x *EchoRequest) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

type EchoResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EchoResponse) Reset() {
	*x = EchoResponse{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EchoResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EchoResponse) ProtoMessage() {}

func (x *EchoResponse) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EchoResponse.ProtoReflect.Descriptor instead.
func (*EchoResponse) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{1}
}

func (x *EchoResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

type ConfigSpec struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Enabled       *bool                  `protobuf:"varint,1,opt,name=enabled,proto3,oneof" json:"enabled,omitempty"`
	Revision      *v1.Revision           `protobuf:"bytes,2,opt,name=revision,proto3" json:"revision,omitempty"`
	StringVar     *string                `protobuf:"bytes,3,opt,name=stringVar,proto3,oneof" json:"stringVar,omitempty"`
	IntVar        *int32                 `protobuf:"varint,4,opt,name=intVar,proto3,oneof" json:"intVar,omitempty"`
	BoolVar       *bool                  `protobuf:"varint,5,opt,name=boolVar,proto3,oneof" json:"boolVar,omitempty"`
	MessageVar    *Message               `protobuf:"bytes,6,opt,name=messageVar,proto3,oneof" json:"messageVar,omitempty"`
	SecretVar     *string                `protobuf:"bytes,7,opt,name=secretVar,proto3,oneof" json:"secretVar,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConfigSpec) Reset() {
	*x = ConfigSpec{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfigSpec) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfigSpec) ProtoMessage() {}

func (x *ConfigSpec) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConfigSpec.ProtoReflect.Descriptor instead.
func (*ConfigSpec) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{2}
}

func (x *ConfigSpec) GetEnabled() bool {
	if x != nil && x.Enabled != nil {
		return *x.Enabled
	}
	return false
}

func (x *ConfigSpec) GetRevision() *v1.Revision {
	if x != nil {
		return x.Revision
	}
	return nil
}

func (x *ConfigSpec) GetStringVar() string {
	if x != nil && x.StringVar != nil {
		return *x.StringVar
	}
	return ""
}

func (x *ConfigSpec) GetIntVar() int32 {
	if x != nil && x.IntVar != nil {
		return *x.IntVar
	}
	return 0
}

func (x *ConfigSpec) GetBoolVar() bool {
	if x != nil && x.BoolVar != nil {
		return *x.BoolVar
	}
	return false
}

func (x *ConfigSpec) GetMessageVar() *Message {
	if x != nil {
		return x.MessageVar
	}
	return nil
}

func (x *ConfigSpec) GetSecretVar() string {
	if x != nil && x.SecretVar != nil {
		return *x.SecretVar
	}
	return ""
}

type ResetRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Revision      *v1.Revision           `protobuf:"bytes,1,opt,name=revision,proto3" json:"revision,omitempty"`
	Mask          *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=mask,proto3" json:"mask,omitempty"`
	Patch         *ConfigSpec            `protobuf:"bytes,3,opt,name=patch,proto3" json:"patch,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ResetRequest) Reset() {
	*x = ResetRequest{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ResetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ResetRequest) ProtoMessage() {}

func (x *ResetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ResetRequest.ProtoReflect.Descriptor instead.
func (*ResetRequest) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{3}
}

func (x *ResetRequest) GetRevision() *v1.Revision {
	if x != nil {
		return x.Revision
	}
	return nil
}

func (x *ResetRequest) GetMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.Mask
	}
	return nil
}

func (x *ResetRequest) GetPatch() *ConfigSpec {
	if x != nil {
		return x.Patch
	}
	return nil
}

type DryRunRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Target        driverutil.Target      `protobuf:"varint,1,opt,name=target,proto3,enum=driverutil.Target" json:"target,omitempty"`
	Action        driverutil.Action      `protobuf:"varint,2,opt,name=action,proto3,enum=driverutil.Action" json:"action,omitempty"`
	Spec          *ConfigSpec            `protobuf:"bytes,3,opt,name=spec,proto3" json:"spec,omitempty"`         // Set
	Revision      *v1.Revision           `protobuf:"bytes,4,opt,name=revision,proto3" json:"revision,omitempty"` // Reset
	Patch         *ConfigSpec            `protobuf:"bytes,5,opt,name=patch,proto3" json:"patch,omitempty"`       // Reset
	Mask          *fieldmaskpb.FieldMask `protobuf:"bytes,6,opt,name=mask,proto3" json:"mask,omitempty"`         // Reset
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DryRunRequest) Reset() {
	*x = DryRunRequest{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DryRunRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DryRunRequest) ProtoMessage() {}

func (x *DryRunRequest) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DryRunRequest.ProtoReflect.Descriptor instead.
func (*DryRunRequest) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{4}
}

func (x *DryRunRequest) GetTarget() driverutil.Target {
	if x != nil {
		return x.Target
	}
	return driverutil.Target(0)
}

func (x *DryRunRequest) GetAction() driverutil.Action {
	if x != nil {
		return x.Action
	}
	return driverutil.Action(0)
}

func (x *DryRunRequest) GetSpec() *ConfigSpec {
	if x != nil {
		return x.Spec
	}
	return nil
}

func (x *DryRunRequest) GetRevision() *v1.Revision {
	if x != nil {
		return x.Revision
	}
	return nil
}

func (x *DryRunRequest) GetPatch() *ConfigSpec {
	if x != nil {
		return x.Patch
	}
	return nil
}

func (x *DryRunRequest) GetMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.Mask
	}
	return nil
}

type DryRunResponse struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	Current          *ConfigSpec            `protobuf:"bytes,1,opt,name=current,proto3" json:"current,omitempty"`
	Modified         *ConfigSpec            `protobuf:"bytes,2,opt,name=modified,proto3" json:"modified,omitempty"`
	ValidationErrors *validate.Violations   `protobuf:"bytes,3,opt,name=validationErrors,proto3" json:"validationErrors,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *DryRunResponse) Reset() {
	*x = DryRunResponse{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DryRunResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DryRunResponse) ProtoMessage() {}

func (x *DryRunResponse) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DryRunResponse.ProtoReflect.Descriptor instead.
func (*DryRunResponse) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{5}
}

func (x *DryRunResponse) GetCurrent() *ConfigSpec {
	if x != nil {
		return x.Current
	}
	return nil
}

func (x *DryRunResponse) GetModified() *ConfigSpec {
	if x != nil {
		return x.Modified
	}
	return nil
}

func (x *DryRunResponse) GetValidationErrors() *validate.Violations {
	if x != nil {
		return x.ValidationErrors
	}
	return nil
}

type HistoryResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Entries       []*ConfigSpec          `protobuf:"bytes,1,rep,name=entries,proto3" json:"entries,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HistoryResponse) Reset() {
	*x = HistoryResponse{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HistoryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HistoryResponse) ProtoMessage() {}

func (x *HistoryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HistoryResponse.ProtoReflect.Descriptor instead.
func (*HistoryResponse) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{6}
}

func (x *HistoryResponse) GetEntries() []*ConfigSpec {
	if x != nil {
		return x.Entries
	}
	return nil
}

type Message struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	StringSliceVar []string               `protobuf:"bytes,1,rep,name=stringSliceVar,proto3" json:"stringSliceVar,omitempty"`
	StringMapVar   map[string]string      `protobuf:"bytes,2,rep,name=stringMapVar,proto3" json:"stringMapVar,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *Message) Reset() {
	*x = Message{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Message) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Message) ProtoMessage() {}

func (x *Message) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Message.ProtoReflect.Descriptor instead.
func (*Message) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{7}
}

func (x *Message) GetStringSliceVar() []string {
	if x != nil {
		return x.StringSliceVar
	}
	return nil
}

func (x *Message) GetStringMapVar() map[string]string {
	if x != nil {
		return x.StringMapVar
	}
	return nil
}

type SetRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Spec          *ConfigSpec            `protobuf:"bytes,1,opt,name=spec,proto3" json:"spec,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetRequest) Reset() {
	*x = SetRequest{}
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetRequest) ProtoMessage() {}

func (x *SetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetRequest.ProtoReflect.Descriptor instead.
func (*SetRequest) Descriptor() ([]byte, []int) {
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP(), []int{8}
}

func (x *SetRequest) GetSpec() *ConfigSpec {
	if x != nil {
		return x.Spec
	}
	return nil
}

var File_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto protoreflect.FileDescriptor

const file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDesc = "" +
	"\n" +
	"Egithub.com/aity-cloud/monty/plugins/example/pkg/example/example.proto\x12\aexample\x1a\x1bbuf/validate/validate.proto\x1a)github.com/kralicky/codegen/cli/cli.proto\x1a7github.com/aity-cloud/monty/pkg/apis/core/v1/core.proto\x1a>github.com/aity-cloud/monty/pkg/plugins/driverutil/types.proto\x1a\x1cgoogle/api/annotations.proto\x1a\x1bgoogle/protobuf/empty.proto\x1a google/protobuf/field_mask.proto\"'\n" +
	"\vEchoRequest\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\"(\n" +
	"\fEchoResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\"\xee\x02\n" +
	"\n" +
	"ConfigSpec\x12%\n" +
	"\aenabled\x18\x01 \x01(\bB\x06\x8a\xc0\f\x02(\x01H\x00R\aenabled\x88\x01\x01\x12*\n" +
	"\brevision\x18\x02 \x01(\v2\x0e.core.RevisionR\brevision\x12!\n" +
	"\tstringVar\x18\x03 \x01(\tH\x01R\tstringVar\x88\x01\x01\x12\x1b\n" +
	"\x06intVar\x18\x04 \x01(\x05H\x02R\x06intVar\x88\x01\x01\x12\x1d\n" +
	"\aboolVar\x18\x05 \x01(\bH\x03R\aboolVar\x88\x01\x01\x125\n" +
	"\n" +
	"messageVar\x18\x06 \x01(\v2\x10.example.MessageH\x04R\n" +
	"messageVar\x88\x01\x01\x12)\n" +
	"\tsecretVar\x18\a \x01(\tB\x06\x8a\xc0\f\x02\x18\x01H\x05R\tsecretVar\x88\x01\x01B\n" +
	"\n" +
	"\b_enabledB\f\n" +
	"\n" +
	"_stringVarB\t\n" +
	"\a_intVarB\n" +
	"\n" +
	"\b_boolVarB\r\n" +
	"\v_messageVarB\f\n" +
	"\n" +
	"_secretVar\"\xad\x01\n" +
	"\fResetRequest\x122\n" +
	"\brevision\x18\x01 \x01(\v2\x0e.core.RevisionB\x06\x8a\xc0\f\x02(\x01R\brevision\x126\n" +
	"\x04mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskB\x06\x8a\xc0\f\x02(\x01R\x04mask\x121\n" +
	"\x05patch\x18\x03 \x01(\v2\x13.example.ConfigSpecB\x06\x8a\xc0\f\x02(\x01R\x05patch\"\x9f\x02\n" +
	"\rDryRunRequest\x12*\n" +
	"\x06target\x18\x01 \x01(\x0e2\x12.driverutil.TargetR\x06target\x12*\n" +
	"\x06action\x18\x02 \x01(\x0e2\x12.driverutil.ActionR\x06action\x12'\n" +
	"\x04spec\x18\x03 \x01(\v2\x13.example.ConfigSpecR\x04spec\x12*\n" +
	"\brevision\x18\x04 \x01(\v2\x0e.core.RevisionR\brevision\x12)\n" +
	"\x05patch\x18\x05 \x01(\v2\x13.example.ConfigSpecR\x05patch\x126\n" +
	"\x04mask\x18\x06 \x01(\v2\x1a.google.protobuf.FieldMaskB\x06\x8a\xc0\f\x02(\x01R\x04mask\"\xb6\x01\n" +
	"\x0eDryRunResponse\x12-\n" +
	"\acurrent\x18\x01 \x01(\v2\x13.example.ConfigSpecR\acurrent\x12/\n" +
	"\bmodified\x18\x02 \x01(\v2\x13.example.ConfigSpecR\bmodified\x12D\n" +
	"\x10validationErrors\x18\x03 \x01(\v2\x18.buf.validate.ViolationsR\x10validationErrors\"@\n" +
	"\x0fHistoryResponse\x12-\n" +
	"\aentries\x18\x01 \x03(\v2\x13.example.ConfigSpecR\aentries\"\xba\x01\n" +
	"\aMessage\x12&\n" +
	"\x0estringSliceVar\x18\x01 \x03(\tR\x0estringSliceVar\x12F\n" +
	"\fstringMapVar\x18\x02 \x03(\v2\".example.Message.StringMapVarEntryR\fstringMapVar\x1a?\n" +
	"\x11StringMapVarEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"5\n" +
	"\n" +
	"SetRequest\x12'\n" +
	"\x04spec\x18\x01 \x01(\v2\x13.example.ConfigSpecR\x04spec2\x95\x01\n" +
	"\x13ExampleAPIExtension\x12E\n" +
	"\x04Echo\x12\x14.example.EchoRequest\x1a\x15.example.EchoResponse\"\x10\x82\xd3\xe4\x93\x02\n" +
	":\x01*\"\x05/echo\x127\n" +
	"\x05Ready\x12\x16.google.protobuf.Empty\x1a\x16.google.protobuf.Empty2O\n" +
	"\x15ExampleUnaryExtension\x126\n" +
	"\x05Hello\x12\x16.google.protobuf.Empty\x1a\x15.example.EchoResponse2\xd3\x05\n" +
	"\x06Config\x12[\n" +
	"\x17GetDefaultConfiguration\x12\x16.driverutil.GetRequest\x1a\x13.example.ConfigSpec\"\x13\x82\xc0\f\x0f\x8a\xc0\f\vget-default\x12_\n" +
	"\x17SetDefaultConfiguration\x12\x13.example.SetRequest\x1a\x16.google.protobuf.Empty\"\x17\x82\xc0\f\x13\x8a\xc0\f\vset-default\xb0\xc0\f\x01\x12L\n" +
	"\x10GetConfiguration\x12\x16.driverutil.GetRequest\x1a\x13.example.ConfigSpec\"\v\x82\xc0\f\a\x8a\xc0\f\x03get\x12P\n" +
	"\x10SetConfiguration\x12\x13.example.SetRequest\x1a\x16.google.protobuf.Empty\"\x0f\x82\xc0\f\v\x8a\xc0\f\x03set\xb0\xc0\f\x01\x12b\n" +
	"\x19ResetDefaultConfiguration\x12\x16.google.protobuf.Empty\x1a\x16.google.protobuf.Empty\"\x15\x82\xc0\f\x11\x8a\xc0\f\rreset-default\x12V\n" +
	"\x12ResetConfiguration\x12\x15.example.ResetRequest\x1a\x16.google.protobuf.Empty\"\x11\x82\xc0\f\r\x8a\xc0\f\x05reset\xb0\xc0\f\x01\x12C\n" +
	"\x06DryRun\x12\x16.example.DryRunRequest\x1a\x17.example.DryRunResponse\"\b\x82\xc0\f\x04\xa0\xc0\f\x01\x12j\n" +
	"\x14ConfigurationHistory\x12'.driverutil.ConfigurationHistoryRequest\x1a\x18.example.HistoryResponse\"\x0f\x82\xc0\f\v\x8a\xc0\f\ahistoryB?\x82\xc0\f\x02\b\x01Z7github.com/aity-cloud/monty/plugins/example/pkg/exampleb\x06proto3"

var (
	file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescOnce sync.Once
	file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescData []byte
)

func file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescGZIP() []byte {
	file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescOnce.Do(func() {
		file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDesc), len(file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDesc)))
	})
	return file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDescData
}

var file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes = make([]protoimpl.MessageInfo, 10)
var file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_goTypes = []any{
	(*EchoRequest)(nil),                            // 0: example.EchoRequest
	(*EchoResponse)(nil),                           // 1: example.EchoResponse
	(*ConfigSpec)(nil),                             // 2: example.ConfigSpec
	(*ResetRequest)(nil),                           // 3: example.ResetRequest
	(*DryRunRequest)(nil),                          // 4: example.DryRunRequest
	(*DryRunResponse)(nil),                         // 5: example.DryRunResponse
	(*HistoryResponse)(nil),                        // 6: example.HistoryResponse
	(*Message)(nil),                                // 7: example.Message
	(*SetRequest)(nil),                             // 8: example.SetRequest
	nil,                                            // 9: example.Message.StringMapVarEntry
	(*v1.Revision)(nil),                            // 10: core.Revision
	(*fieldmaskpb.FieldMask)(nil),                  // 11: google.protobuf.FieldMask
	(driverutil.Target)(0),                         // 12: driverutil.Target
	(driverutil.Action)(0),                         // 13: driverutil.Action
	(*validate.Violations)(nil),                    // 14: buf.validate.Violations
	(*emptypb.Empty)(nil),                          // 15: google.protobuf.Empty
	(*driverutil.GetRequest)(nil),                  // 16: driverutil.GetRequest
	(*driverutil.ConfigurationHistoryRequest)(nil), // 17: driverutil.ConfigurationHistoryRequest
}
var file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_depIdxs = []int32{
	10, // 0: example.ConfigSpec.revision:type_name -> core.Revision
	7,  // 1: example.ConfigSpec.messageVar:type_name -> example.Message
	10, // 2: example.ResetRequest.revision:type_name -> core.Revision
	11, // 3: example.ResetRequest.mask:type_name -> google.protobuf.FieldMask
	2,  // 4: example.ResetRequest.patch:type_name -> example.ConfigSpec
	12, // 5: example.DryRunRequest.target:type_name -> driverutil.Target
	13, // 6: example.DryRunRequest.action:type_name -> driverutil.Action
	2,  // 7: example.DryRunRequest.spec:type_name -> example.ConfigSpec
	10, // 8: example.DryRunRequest.revision:type_name -> core.Revision
	2,  // 9: example.DryRunRequest.patch:type_name -> example.ConfigSpec
	11, // 10: example.DryRunRequest.mask:type_name -> google.protobuf.FieldMask
	2,  // 11: example.DryRunResponse.current:type_name -> example.ConfigSpec
	2,  // 12: example.DryRunResponse.modified:type_name -> example.ConfigSpec
	14, // 13: example.DryRunResponse.validationErrors:type_name -> buf.validate.Violations
	2,  // 14: example.HistoryResponse.entries:type_name -> example.ConfigSpec
	9,  // 15: example.Message.stringMapVar:type_name -> example.Message.StringMapVarEntry
	2,  // 16: example.SetRequest.spec:type_name -> example.ConfigSpec
	0,  // 17: example.ExampleAPIExtension.Echo:input_type -> example.EchoRequest
	15, // 18: example.ExampleAPIExtension.Ready:input_type -> google.protobuf.Empty
	15, // 19: example.ExampleUnaryExtension.Hello:input_type -> google.protobuf.Empty
	16, // 20: example.Config.GetDefaultConfiguration:input_type -> driverutil.GetRequest
	8,  // 21: example.Config.SetDefaultConfiguration:input_type -> example.SetRequest
	16, // 22: example.Config.GetConfiguration:input_type -> driverutil.GetRequest
	8,  // 23: example.Config.SetConfiguration:input_type -> example.SetRequest
	15, // 24: example.Config.ResetDefaultConfiguration:input_type -> google.protobuf.Empty
	3,  // 25: example.Config.ResetConfiguration:input_type -> example.ResetRequest
	4,  // 26: example.Config.DryRun:input_type -> example.DryRunRequest
	17, // 27: example.Config.ConfigurationHistory:input_type -> driverutil.ConfigurationHistoryRequest
	1,  // 28: example.ExampleAPIExtension.Echo:output_type -> example.EchoResponse
	15, // 29: example.ExampleAPIExtension.Ready:output_type -> google.protobuf.Empty
	1,  // 30: example.ExampleUnaryExtension.Hello:output_type -> example.EchoResponse
	2,  // 31: example.Config.GetDefaultConfiguration:output_type -> example.ConfigSpec
	15, // 32: example.Config.SetDefaultConfiguration:output_type -> google.protobuf.Empty
	2,  // 33: example.Config.GetConfiguration:output_type -> example.ConfigSpec
	15, // 34: example.Config.SetConfiguration:output_type -> google.protobuf.Empty
	15, // 35: example.Config.ResetDefaultConfiguration:output_type -> google.protobuf.Empty
	15, // 36: example.Config.ResetConfiguration:output_type -> google.protobuf.Empty
	5,  // 37: example.Config.DryRun:output_type -> example.DryRunResponse
	6,  // 38: example.Config.ConfigurationHistory:output_type -> example.HistoryResponse
	28, // [28:39] is the sub-list for method output_type
	17, // [17:28] is the sub-list for method input_type
	17, // [17:17] is the sub-list for extension type_name
	17, // [17:17] is the sub-list for extension extendee
	0,  // [0:17] is the sub-list for field type_name
}

func init() { file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_init() }
func file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_init() {
	if File_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto != nil {
		return
	}
	file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes[2].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDesc), len(file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   10,
			NumExtensions: 0,
			NumServices:   3,
		},
		GoTypes:           file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_goTypes,
		DependencyIndexes: file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_depIdxs,
		MessageInfos:      file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_msgTypes,
	}.Build()
	File_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto = out.File
	file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_goTypes = nil
	file_github_com_aity_cloud_monty_plugins_example_pkg_example_example_proto_depIdxs = nil
}
