...
import (
	...
	errors1 "github.com/kralicky/codegen/pkg/errors"
...
type (
	contextKey_ExampleAPIExtension_type      struct{}
	contextInjector_ExampleAPIExtension_type struct{}
)

var (
	contextKey_ExampleAPIExtension     contextKey_ExampleAPIExtension_type
	ExampleAPIExtensionContextInjector contextInjector_ExampleAPIExtension_type
)

func (contextInjector_ExampleAPIExtension_type) NewClient(cc grpc.ClientConnInterface) ExampleAPIExtensionClient {
...
func (contextInjector_ExampleAPIExtension_type) ClientFromContext(ctx context.Context) (ExampleAPIExtensionClient, bool) {
	client, ok := ctx.Value(contextKey_ExampleAPIExtension).(ExampleAPIExtensionClient)
	return client, ok
}

type (
	contextKey_ExampleUnaryExtension_type      struct{}
	contextInjector_ExampleUnaryExtension_type struct{}
)

var (
	contextKey_ExampleUnaryExtension     contextKey_ExampleUnaryExtension_type
	ExampleUnaryExtensionContextInjector contextInjector_ExampleUnaryExtension_type
)

func (contextInjector_ExampleUnaryExtension_type) NewClient(cc grpc.ClientConnInterface) ExampleUnaryExtensionClient {
...
func (contextInjector_ExampleUnaryExtension_type) ClientFromContext(ctx context.Context) (ExampleUnaryExtensionClient, bool) {
...
`[1:],
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			client, ok := ExampleAPIExtensionContextInjector.ClientFromContext(cmd.Context())
			if !ok {
...
func BuildExampleUnaryExtensionHelloCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		ValidArgsFunction: cobra.NoFileCompletions,
		RunE: func(cmd *cobra.Command, args []string) error {
			client, ok := ExampleUnaryExtensionContextInjector.ClientFromContext(cmd.Context())
			if !ok {
				cmd.PrintErrln("failed to get client from context")
				return nil
			}
...
func (in *SetRequest) UnredactSecrets(unredacted *SetRequest) error {
	...
	if err := in.Spec.UnredactSecrets(unredacted.GetSpec()); errors1.IsDiscontinuity(err) {
