...
import (
	errors "github.com/kralicky/codegen/pkg/errors"
...
func (in *Config) UnredactSecrets(unredacted *Config) error {
	...
	if err := in.S3.UnredactSecrets(unredacted.GetS3()); errors.IsDiscontinuity(err) {
	...
	if err := in.Gcs.UnredactSecrets(unredacted.GetGcs()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
	...
	if err := in.Azure.UnredactSecrets(unredacted.GetAzure()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
...
func (in *S3Config) FlagSet(prefix ...string) *pflag.FlagSet {
	...
	fs.Var(flagutil.StringPtrValue(nil, &in.ListObjectsVersion), strings.Join(append(prefix, "list-objects-version"), "."), "The list api version. Supported values are: v1, v2, and ''.")
	fs.Var(flagutil.BoolPtrValue(flagutil.Ptr(false), &in.DisableDualstack), strings.Join(append(prefix, "disable-dualstack"), "."), "If enabled, S3 endpoint will use the non-dualstack variant.")
	fs.Var(flagutil.BoolPtrValue(flagutil.Ptr(true), &in.SendContentMd5), strings.Join(append(prefix, "send-content-md-5"), "."), "If true, attach MD5 checksum when upload objects and S3 uses MD5 checksum algorithm to verify the provided digest. If false, use CRC32C algorithm instead.")
	return fs
}
...
func (in *S3Config) UnredactSecrets(unredacted *S3Config) error {
	...
	if err := in.Sse.UnredactSecrets(unredacted.GetSse()); errors.IsDiscontinuity(err) {
		for _, sd := range status.Convert(err).Details() {
			if info, ok := sd.(*errdetails.ErrorInfo); ok {
...
func (in *AzureConfig) FlagSet(prefix ...string) *pflag.FlagSet {
	...
	fs.MarkDeprecated(strings.Join(append(prefix, "msi-resource"), "."), "\x1b[31m[secret]\x1b[0m Deprecated: Azure storage MSI resource. It will be set automatically by Azure SDK.")
	fs.Var(flagutil.StringPtrValue(nil, &in.MsiResource), strings.Join(append(prefix, "msi-resource"), "."), "\x1b[31m[secret]\x1b[0m Deprecated: Azure storage MSI resource. It will be set automatically by Azure SDK.")
...
func (in *SwiftConfig) FlagSet(prefix ...string) *pflag.FlagSet {
	...
	fs.Var(flagutil.StringPtrValue(nil, &in.ApplicationCredentialSecret), strings.Join(append(prefix, "application-credential-secret"), "."), "OpenStack Swift application credential secret.")
	...
	fs.Var(flagutil.StringPtrValue(nil, &in.Password), strings.Join(append(prefix, "password"), "."), "OpenStack Swift API key.")
